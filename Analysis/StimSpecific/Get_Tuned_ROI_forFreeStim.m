...
switch s.Pattern
    ...
    case {'Moving Bar', 'Shifting Grating'}
        ...
        for i_ROI = im.roi_res
            ...
            %Judge
            if sum(DSI_shuffle(1,:) > im.L_DS(1, i_ROI))/n_permutation < 0.05
                roi_DS_p = [roi_DS_p, i_ROI];
            end

            if sum(OSI_shuffle(1,:) > im.L_OS(1, i_ROI))/n_permutation < 0.05
                roi_OS_p = [roi_OS_p, i_ROI];
            end

            if sum(DSI_shuffle(2,:) > im.L_DS(1, i_ROI))/n_permutation < 0.05
                roi_DS_n = [roi_DS_n, i_ROI];
            end

            if sum(OSI_shuffle(2,:) > im.L_OS(1, i_ROI))/n_permutation < 0.05
        ...
        im.roi_non_selective = ...
            setdiff(im.roi_res, union(im.roi_DS, im.roi_OS));
